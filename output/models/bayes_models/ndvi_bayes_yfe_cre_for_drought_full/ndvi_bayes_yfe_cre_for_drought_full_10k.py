...
ndvi_regression_data_insample = pd.read_csv("data/regression/ndvi_regression_data.csv").dropna().reset_index(drop=True)
countries_in_dataset = set(ndvi_regression_data_insample.country)
years_in_dataset = set(ndvi_regression_data_insample.year)
country_fe_cols = [col for col in ndvi_regression_data_insample.columns if "country_fixed_effect" in col]
year_fe_cols = [col for col in ndvi_regression_data_insample.columns if "year_fixed_effect" in col]

countries_to_remove = []
for country_fe in country_fe_cols:
    country = country_fe.split("_")[0]
    if country not in countries_in_dataset:
        ndvi_regression_data_insample = ndvi_regression_data_insample.drop(country_fe, axis=1)
...
for year_fe in year_fe_cols:
    ...
    if int(year) not in years_in_dataset:
        ndvi_regression_data_insample = ndvi_regression_data_insample.drop(year_fe, axis=1)
...
for covar_col in model_spec["continuous_covariates"]:
    ...
    ndvi_regression_data_insample[covar_col.replace("[weight]",model_spec["weights"])+"_scaled"] = covar_scalers[-1].fit_transform(np.array(ndvi_regression_data_insample[covar_col.replace("[weight]",model_spec["weights"])]).reshape(-1,1)).flatten()
target_var_scaler = StandardScaler()
ndvi_regression_data_insample[model_spec["target"]+"_scaled"] = target_var_scaler.fit_transform(np.array(ndvi_regression_data_insample[model_spec["target"]]).reshape(-1,1)).flatten()

target_data = ndvi_regression_data_insample[model_spec["target"]+"_scaled"]
model_variables = []
...
for fe in model_spec["fixed_effects"]:
    for fe_col in [col for col in ndvi_regression_data_insample.columns if col.endswith(f"{fe}_fixed_effect")]:
        model_variables.append(fe_col)
model_data = ndvi_regression_data_insample[model_variables]
...
with pm.Model() as pymc_model:
    ...
    country_rs_matrix = pm.Deterministic("country_rs_matrix", pt.sum(country_rs_coefs * ndvi_regression_data_insample[country_fe_cols],axis=1))
    ...
    ndvi_prior = pm.Deterministic(
        "ndvi_prior",
        covar_terms +
        drought_terms
    )
    
    ndvi_std_scale = pm.HalfNormal("ndvi_std_scale", 10)
    ndvi_std = pm.HalfNormal("ndvi_std", sigma=ndvi_std_scale)
    ndvi_posterior = pm.Normal('ndvi_posterior', mu=ndvi_prior, sigma=ndvi_std, observed=target_data)

    prior = pm.sample_prior_predictive()
    trace = pm.sample(target_accept=.99, cores=4, tune=5000, draws=5000)
    posterior = pm.sample_posterior_predictive(trace, extend_inferencedata=True)

with open ('output/models/bayes_models/ndvi_bayes_yfe_cre_for_drought_full_10k.pkl', 'wb') as buff:
    pkl.dump({
        "prior":prior,
        "trace":trace,
        "posterior":posterior,
